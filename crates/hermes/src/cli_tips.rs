...
//!
//! Native Rust port of `hermes_cli/tips.py`. The Python module exposed a flat
//! list `TIPS` and a single helper `get_random_tip(exclude_recent=0)` that
...
/// The full corpus of CLI tips. Order matches `hermes_cli/tips.py`.
pub static TIPS: &[&str] = &[
    // --- Slash Commands ---
...
///
/// `exclude_recent` is not used currently; it is reserved for future
...
mod tests {
    ...
    #[test]
    fn corpus_count_matches_python() {
        // The Python TIPS list had this exact number of entries.
        assert_eq!(TIPS.len(), 380);
    }

    #[test]
    fn no_blank_tips() {
        for (i, t) in TIPS.iter().enumerate() {
            assert!(!t.trim().is_empty(), "tip {i} is blank");
        }
    }

    #[test]
    fn get_random_tip_returns_member() {
        for _ in 0..200 {
            let tip = get_random_tip(0);
            assert!(TIPS.contains(&tip), "returned tip not in corpus: {tip:?}");
        }
    }

    #[test]
    fn exclude_recent_arg_is_ignored() {
        // Any value is accepted and behaves identically (reserved arg).
        let tip = get_random_tip(5);
        assert!(TIPS.contains(&tip));
    }

    #[test]
    fn first_and_last_tips_are_stable() {
        assert_eq!(
            TIPS[0],
            "/background <prompt> (alias /bg or /btw) runs a task in a separate session while your current one stays free."
        );
