...
//! `"arguments"`. Also handles an unclosed `<tool_call>` at end-of-string
//! (truncated generation).
//!
//! Port of `environments/tool_call_parsers/hermes_parser.py`.
...
pub struct ToolCall {
    /// Hermes tool call IDs look like `call_<8 hex chars>`.
    pub id: String,
    /// Always `"function"`.
...
/// - `content`: the text before the first `<tool_call>` tag, or `None` if empty.
/// - `tool_calls`: the parsed tool calls, or `None` if none were found.
pub type ParseResult = (Option<String>, Option<Vec<ToolCall>>);

/// The opening tag marking the start of a Hermes tool call.
pub const TOOL_CALL_TAG: &str = "<tool_call>";

/// Matches both closed and unclosed tool_call tags.
///
/// Mirrors the Python regex
/// `r"<tool_call>\s*(.*?)\s*</tool_call>|<tool_call>\s*(.*)"` compiled with
/// `re.DOTALL`. `(?s)` enables dotall so `.` matches newlines.
fn pattern() -> &'static Regex {
    static PATTERN: OnceLock<Regex> = OnceLock::new();
    PATTERN.get_or_init(|| {
        Regex::new(r"(?s)<tool_call>\s*(.*?)\s*</tool_call>|<tool_call>\s*(.*)")
            .expect("valid hermes tool_call regex")
    })
}

const ID_HEX: &[u8] = b"0123456789abcdef";

/// Hermes tool call IDs are `call_` followed by 8 hex chars (first 8 of a uuid4 hex).
pub fn generate_hermes_id() -> String {
    use std::cell::Cell;
...
impl HermesToolCallParser {
    ...
    pub fn parse(&self, text: &str) -> ParseResult {
        if !text.contains(TOOL_CALL_TAG) {
...
fn parse_inner(text: &str) -> Option<ParseResult> {
    ...
    for caps in re.captures_iter(text) {
        ...
        let name = match tc_data.get("name") {
            ...
            None => continue, // "name" not in tc_data
        };
        ...
        tool_calls.push(ToolCall::function(generate_hermes_id(), name, arguments));
    }
    ...
    let content = match text.find(TOOL_CALL_TAG) {
...
mod tests {
    ...
    fn no_tag_returns_text() {
        let p = HermesToolCallParser::new();
    ...
    fn content_before_tool_call_kept() {
        let p = HermesToolCallParser::new();
        let text = "Let me check.  <tool_call>{\"name\": \"f\", \"arguments\": {}}</tool_call>";
        let (content, calls) = p.parse(text);
    ...
    fn unclosed_tool_call_at_end() {
        let p = HermesToolCallParser::new();
        let text = "<tool_call>{\"name\": \"trunc\", \"arguments\": {\"k\": \"v\"}}";
        let (content, calls) = p.parse(text);
        assert_eq!(content, None);
        let calls = calls.unwrap();
        assert_eq!(calls.len(), 1);
    ...
    #[test]
    fn missing_name_aborts_via_no_calls() {
        // The single tool call has no "name", so it is skipped; with no calls
        // collected, the original text is returned.
        let p = HermesToolCallParser::new();
    ...
    fn non_ascii_arguments_preserved() {
        let p = HermesToolCallParser::new();
        let text = "<tool_call>{\"name\": \"f\", \"arguments\": {\"city\": \"São Paulo\"}}</tool_call>";
        let (_content, calls) = p.parse(text);
        let calls = calls.unwrap();
