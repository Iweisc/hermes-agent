...
def _discard_background_task(task_input):
    if asyncio.iscoroutine(task_input):
        task_input.close()
    return MagicMock(name="socket-mode-task")
...
class TestAppMentionHandler:
    ...
    async def test_app_mention_registered_on_connect(self):
        ...
        with (
            ...
            patch("asyncio.create_task", side_effect=_discard_background_task),
        ):
